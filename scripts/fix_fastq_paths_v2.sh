...
# 
# Usage: 
# ./fix_fastq_paths.sh <flow cell run folder>
#
################################################################################
...
do
  # Move library folders to Project directory
  tempFolder=`ls $dataFolder/$projFolder | grep "^[0-9]*"`
  mv -r $dataFolder/$projFolder/$tempFolder/lib* $dataFolder/$projFolder/
  ...
  rm -r $dataFolder/$projFolder/$tempFolder
done
...
class ResultCurator(object):
    def __init__(self, processed_dir=None, batch_file=None):
    ...
    def get_outputs(self):
        ...
        output_dict = WorkflowParser(self.bf).show_output_files()
...
def check_outputs(processed_dir, batch_file):
    project_dir = os.path.basename(os.path.abspath(processed_dir))
    print project_dir

    rc = ResultCurator(processed_dir, batch_file)
    ...
    missing_outputs = [ f for f in rc.list_outputs() \
                        if f not in project_files and processed_dir in f]
...
def main(argv):
    ...
    missing,empty = check_outputs(processed_dir, batch_file)
